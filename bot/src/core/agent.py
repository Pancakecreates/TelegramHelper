...
11.1) "add_reminder" — поставить персональное напоминание (Commitment direction="mine").
    Параметры:
      ...
      "start_at": "ISO-8601 datetime в UTC (время начала задачи, если это интервал/график, например с 15:00 до 16:00, тогда start_at будет в 15:00. Если интервала нет, то null)",
      "when": "ISO-8601 datetime в UTC (например 2026-05-10T15:00:00Z) или null если нет даты. Если это интервал, это время окончания)",
    ...
    через час сделать X», «не забыть Y», «спланировать задачу с 15:00 до 16:00».
